...
inline vec3 random_in_unit_sphere() {
    // random sampling is done as the alternative analytical methods are complicated.
    while (true) {
...
inline vec3 random_on_hemisphere(const vec3& normal) {
    ...
    if (dot(vec_on_unit_sphere, normal) > 0.0) { // In the same hemisphere as the normal
